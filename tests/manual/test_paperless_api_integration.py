...
from bank_statement_separator.utils.paperless_client import PaperlessClient, PaperlessUploadError
...
def validate_api_connection():
    ...
    try:
        ...
        if result["documents"]:
            ...
            print(f"📄 Sample document: ID={doc['id']}, Title='{doc.get('title', 'N/A')}'")
...
def create_test_data():
    ...
    try:
        ...
        try:
            correspondent_id = client._resolve_correspondent("Test Bank API Integration")
            if correspondent_id:
                print(f"✅ Correspondent 'Test Bank API Integration' available (ID: {correspondent_id})")
        except Exception as e:
        ...
        try:
            ...
            if doc_type_id:
                print(f"✅ Document type 'Test Statement Integration' available (ID: {doc_type_id})")
        except Exception as e:
...
def main():
    ...
    parser.add_argument(
        "--setup",
        action="store_true", 
        help="Set up test environment and directories"
    )
    ...
    parser.add_argument(
        "--cleanup",
        action="store_true",
        help="Clean up test data and temporary files"
    )
    ...
    if not any([args.setup, args.validate, args.create_data, args.run_tests, args.cleanup]):
        print("🔧 Paperless-ngx API Integration Test Helper")
        print("Use --help for options or --help-detailed for comprehensive guide")
        print("\n🚀 Quick start: python {} --setup --validate --run-tests".format(sys.argv[0]))
